...
class FloatingButtonController: UIViewController {
    ...
    override func loadView() {
        ...
        let bottomSpace = CustomerGlu.getInstance.floatingVerticalPadding
        let sideSpace = CustomerGlu.getInstance.floatingHorizontalPadding
  ...
  @objc func performButtonAction() {
      if let actionData = floatInfo?.mobile.content[0].action, let type = actionData.type {
          if type == WebViewsKey.open_deeplink {
              ...
              nudgeConfiguration.closeOnDeepLink = floatInfo?.mobile.content[0].closeOnDeepLink ?? CustomerGlu.getInstance.auto_close_webview!
