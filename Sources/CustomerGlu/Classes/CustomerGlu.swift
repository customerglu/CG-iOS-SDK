...
//        CGExceptionHelper.shared.logoutSentryUser() TBD
...
//                CGExceptionHelper.shared.setupSentry() TBD
            }
    ...
    @objc public func registerDevice(userdata: [String: AnyHashable], completion: @escaping (Bool) -> Void) {
        ...
        APIManager.userRegister(queryParameters: userData as NSDictionary) { result in
            ...
            case .success(let response):
                if response.success! {
                    // Setup Sentry user
//                    CGExceptionHelper.shared.setupUser(userId: response.data?.user?.userId ?? "", clientId: response.data?.user?.client ?? "") TBD
